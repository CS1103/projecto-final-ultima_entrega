...
public:
    ...
    Tensor2<T> forward(const Tensor2<T>& X) {
        auto out = X;
        for (auto& l : layers)
            out = l->forward(out);
        return out;
    ...
    /// Realiza backward a partir de la pérdida calculada
    ...
    /// Ajuste de parámetros en cada capa Dense
    void optimize(T lr) {
        for (auto& l : layers) {
    ...
    /// Entrena en batch X, Y por epochs iteraciones
    void train(const Tensor2<T>& X, const Tensor2<T>& Y,
               size_t epochs, T lr) 
    {
